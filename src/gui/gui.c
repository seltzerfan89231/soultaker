...
void gui_init(void)
{
    ...
    gui.length = 0;
    gui.root = component_create(0.0f, 0.0f, 1.0f, 1.0f, 0, 0.0f);
    gui.root->interactable = 0;
    Component *btn = component_create(0.1, 0.5, 0.4, 0.4, 0.5f, 1);
...
void gui_push_data_helper(f32 ar, Component *comp, f32 x, f32 y, f32 w, f32 h)
{
    ...
    for (i32 i = 0; i < comp->num_children; i++)
        gui_push_data_helper(ar, comp->children[i], new_x1, new_y1, new_x2 - new_x1, new_y2 - new_y1);
}

void gui_push_data(f32 ar)
{
    gui.length = 0;
    gui_push_data_helper(ar, gui.root, gui.root->x, gui.root->y, gui.root->w, gui.root->h);
}

bool gui_interact_helper(f32 ar, vec2f mouse_pos, Component *comp, f32 x, f32 y, f32 w, f32 h)
{
    f32 new_x1, new_y1, new_x2, new_y2, win_x1, win_x2, win_y1, win_y2;
    new_x1 = comp->x * w + x, new_x2 = (comp->x + comp->w) * w + x;
    new_y1 = comp->y * h + y, new_y2 = (comp->y + comp->h) * h + y;
    for (i32 i = 0; i < comp->num_children; i++)
        if (gui_interact_helper(ar, mouse_pos, comp->children[i], new_x1, new_y1, new_x2 - new_x1, new_y2 - new_y1))
            return 1;
...
{
    mouse_pos.x *= ar;
    return gui_interact_helper(ar, mouse_pos, gui.root, gui.root->x, gui.root->y, gui.root->w, gui.root->h);
}
