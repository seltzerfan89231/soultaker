...
void game_setup(void) {
    game_insert(data_create(drawable_create(vec3f_create(0, 0, 0), vec3f_create(0.5, 0.5, 0.8), entity_create(PLAYER), ENTITY), QUAD_DATA_LENGTH, game.buffer_length));
...
void game_set_target(vec3f target)
{
    ...
    drawable_vertex_data(game.buffer, game.objects.head->data->val, game.objects.head->data->offset, game.rotation, game.tilt);
}
